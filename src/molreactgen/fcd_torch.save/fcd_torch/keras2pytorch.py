...
def convert_keras2pytorch(keras_model):
    ...
    for layer in keras_model.layers:
        ...
        if "conv1d" in name:
            pytorch_config.append(("Conv1d", convert_Conv1d_layer(layer)))
        elif "lstm" in name:
            pytorch_config.append(("LSTM", convert_LSTM_layer(layer)))
    return pytorch_config


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--keras_model", type=str, help="Path to Keras model (.h5)")
    parser.add_argument(
        "--pytorch_model", type=str, help="Path to output Pytorch model (.pt)"
    )
